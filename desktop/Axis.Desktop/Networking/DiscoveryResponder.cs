...
public sealed class DiscoveryResponder(Action<string> onDiscovery) : IAsyncDisposable
{
    ...
    public async Task RunAsync()
    {
        ...
        while (!_stop.IsCancellationRequested)
        {
            ...
            // 1. Send back to caller's remote endpoint (ephemeral port)
            try { await _socket.SendAsync(response, response.Length, received.RemoteEndPoint).ConfigureAwait(false); } catch { }

            // 2. Also send directly to discovery port 45100 on target IP in case ephemeral port is filtered
            try
            {
                var directTarget = new IPEndPoint(received.RemoteEndPoint.Address, Port);
                await _socket.SendAsync(response, response.Length, directTarget).ConfigureAwait(false);
            }
            catch { }
    ...
    private static bool TryReadDiscovery(ReadOnlySpan<byte> data, out long nonce)
    {
        ...
        nonce = BinaryPrimitives.ReadInt64BigEndian(payload);
        return true;
    }
    ...
    private static byte[] CreateResponsePayload(long nonce)
    {
        ...
        if (name.Length is 0 or > 64) name = Encoding.UTF8.GetBytes("AxisPC");
        ...
        result[12] = (byte)name.Length;
        name.CopyTo(result, 13);
        return result;
    }

    private static byte[] CreatePacket(byte type, byte[] payload)
    {
        var packet = new byte[24 + payload.Length];
        var span = packet.AsSpan();
        BinaryPrimitives.WriteUInt32BigEndian(span, 0x52434C4B);
        span[4] = 1;
        span[5] = type;
    ...
    private static bool TryReadPacket(ReadOnlySpan<byte> packet, out byte type, out ReadOnlySpan<byte> payload)
    {
        ...
        type = packet[5];
        payload = packet.Slice(20, length);
        return true;
    }
